...
parser.add_argument(
    "--cutoff_species", help="cutoff for finest taxonomic level", type=int, default=97
)
parser.add_argument(
    "--cutoff_family", help="cutoff for family taxonomic level", type=int, default=90
)
parser.add_argument(
    "--cutoff_phylum",
    help="cutoff for phylum taxonomic level, also acts as ultimate cutoff value for blast",
    type=int,
    default=80,
)
...
parser.add_argument(
    "--length_percentage",
    help="cutoff for query_hit/length_of_query i.e. query coverage",
    type=float,
    default=0.0,
)
parser.add_argument(
    "--length_cutoff", help="primary cutoff for length of hit", type=int, default=0
)
parser.add_argument(
    "--hits_to_consider",
    help="number of hits to consider when gathering consensus taxonomy",
    type=float,
    default=3,
)
parser.add_argument(
    "--percent_sway",
    help="when comparing greater than 1 blast hit, value is the percent from best hit considered. i.e. if best blast hit is 99.5 ID, a value of 0.5 will consider everything 99 and greater when creating the consensus",
    type=float,
    default=0.5,
)
...
parser.add_argument(
    "--blast_evalue",
    help="setting for e-value cutoff for blast, must be in form 1e-X",
    type=str,
    default="1e-10",
)
parser.add_argument(
    "--blast_threads",
    help="set the number of threads for blast",
    type=str,
    default="24",
)
parser.add_argument(
    "--blast_flavor",
    help="select blastp, tblastn etc, must have correct database and query sequence formats",
    type=str,
    default="blastn",
)
parser.add_argument(
    "--blast_database",
    help="path tp blast database, be sure to run makeblastdb on the database first, type 'IGNORE' if precomputed blast is given",
)
parser.add_argument(
    "--blast_file",
    help="precomputed blast results, MUST BE MY CUSTOMIZED FORMAT, '6 qseqid qlen sseqid pident length qstart qend sstart send evalue bitscore'",
)

# Other options
parser.add_argument(
    "--ncbi_nt", help="REQUIRED flag for use of ncbi nt database", action="store_true"
)
parser.add_argument(
    "--output_dir", help="output directory name", type=str, default="Assigned_Taxonomy"
)
parser.add_argument(
    "--config_file",
    help="NOT YET IMPLEMENTED, an optional parameters file, any arguments in this file will override other options",
)

# REQUIRED ARGUMENTS
parser.add_argument(
    "sequence_file",
    help="seqs.fna file from qiime or any multifasta, just make sure header has unique id with a space",
)

parser.add_argument(
    "tax_file", help="path to silva or customized blast database taxonomy file"
)
...
if args.blast_file == None and args.blast_database == None:
    print(
        "The argument --blast_file or --blast_database is required. You must provide at least one."
    )
    sys.exit()
...
taxonomy_assignment_seven_levels_outfile = (
    outdir + "taxonomy_assignment_per_sequence_eight_levels.tsv"
)
# config_out_handle = open(outdir + 'config_file.txt','w') # might add support for this in the future.


def log_and_print(
    statement, log_file_name_handle=log_file_handle, print_bool=args.verbose
):
...
log_and_print(
    ...
    + str(species_level)
    + " = "
    + taxonomy_categories[species_level]
)
log_and_print(
    "Family_level =  " + str(family_level) + " = " + taxonomy_categories[family_level]
)
log_and_print(
    "Phylum_level =  "
    + str(phylum_level)
    + " = "
    + taxonomy_categories[phylum_level]
    + "\n"
)
...
if args.blast_file == None:
    log_and_print(
        "Running blast on input sequences (this step takes the longest, maybe run overnight)"
    )
...
log_and_print(
    "Parsing taxonomy file and storing info in dictionary (might take a moment if using nt database)"
)
for line in open(args.tax_file, "r"):
    elements = line.rstrip().split("\t")
    tax_id = elements[0]
    tax_info = (
        re.sub("D_[0-9]*__", "", elements[1]).replace(" ", "_").split(";")
    )  # reformat qiime tax file
...
def Assign_Taxonomy(current_query, current_best_hits):
    """the leg work"""
    log_and_print(
        "\nASSIGNING TAXONOMY FOR "
        + current_query
        + " total hits passing initial filters = "
        + str(len(current_best_hits))
    )
    ...
    for key, value in current_best_hits.items():
        if (
            float(value[0]) > (max(top_hits)) - args.percent_sway
        ):  # *****important cutoff
            best_blast_dictionary[key] = value
            log_and_print(
                value[1] + " " + value[0] + " --> CAPTURED after percent sway filter"
            )
    ...
    log_and_print(
        "## Providing consensus taxonomy up to level "
        + str(level)
        + " : "
        + taxonomy_categories[level]
    )
    ...
    for key, value in best_blast_dictionary.items():
        ...
        if (
            value[1] not in taxonomy_dictionary.keys()
        ):  # make sure that the key is in the dictionary
            taxonomy_dictionary[value[1]] = taxonomy_categories
            pkey = "X" + pkey
        if "uncultured" in " ".join(
            taxonomy_dictionary[value[1]][: uncultured_cutoff_level + 1]
        ) or "unidentified" in " ".join(
            taxonomy_dictionary[value[1]][: uncultured_cutoff_level + 1]
        ):
            pkey = "X" + pkey
    ...
    for i in range(
        level + 1
    ):  # start at kingdom and move toward species until difference is met.
    ...
    best_level_taxonomy = list(
        filter(bool, best_level_taxonomy)
    )  # remove empty taxonomic levels
    needed_unknowns = len(taxonomy_categories) - len(
        best_level_taxonomy
    )  # Add undetermined
...
for line in open(blast_file, "r"):
    elements = line.rstrip().split("\t")
    sequence_id = elements[0]
    tax_code = elements[2]
    percent_id = elements[3]
    length_hit = elements[4]
    length_query = elements[1]
    ncbi_id = elements[-1]
    ...
    if current_query != sequence_id:  # INITIATION of new query
        ...
        if bool(current_best_hits) and current_query:  # ENSURE A SET OF BLASTS TO PARSE
            ...
            best_level_taxonomy, blast_percent = Assign_Taxonomy(
                current_query, current_best_hits
            )
            ...
            log_and_print(
                "\nTaxonomy Assignment for "
                + current_query
                + " = "
                + ":".join(best_level_taxonomy)
                + "\n\n\n######"
            )
    ...
    if (
        int(length_hit) / int(length_query) > args.length_percentage
        and float(percent_id) > args.cutoff_phylum
        and int(length_hit) > args.length_cutoff
    ):
        if len(current_best_hits.keys()) < args.hits_to_consider:
            log_and_print("#BLAST LINE : " + line.rstrip())
            label = len(
                current_best_hits.keys()
            )  # provide unique key based on number of matched
...
for seq in sequence_dict.keys():
    ...
    if seq in sequence_taxonomy_dict.keys():
        ...
        with open(taxonomy_assignment_seven_levels_outfile, "a") as t:
            ...
            t.writelines(
                seven_levels_out_line + " " + ":".join(top_hit_dict[seq]) + "\n"
            )
...
log_and_print(
    "Total number of sequences in blast_file: " + str(total_sequences_in_blast)
)
log_and_print(
    "Total number sequences assigned to taxonomy: " + str(total_sequences_assigned)
)
log_and_print(
    "Percentage sequences with assigned taxonomy: "
    + str(total_sequences_assigned / total_sequences * 100)
    + "%"
)
...
os.system(
    "sort -k2 "
    + outdir
    + "taxonomy_assignment_per_sequence.tsv > "
    + outdir
    + "tmp.txt && mv "
    + outdir
    + "tmp.txt "
...
os.system(
    "sort -k2 "
    + outdir
    + "taxonomy_assignment_per_sequence_eight_levels.tsv > "
    + outdir
    + "tmp.txt && mv "
    + outdir
    + "tmp.txt "
